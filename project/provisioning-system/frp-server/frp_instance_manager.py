...
class FrpManager:
    ...
    def _save_instance(self, inst: Dict[str, Any]):
        if self.redis:
            ...
            self.redis.hset("frp:port_allocations", inst['tunnel_game_port'], inst['server_id'])
            if inst.get('tunnel_query_port'):
                self.redis.hset("frp:port_allocations", inst['tunnel_query_port'], inst['server_id'])
...
def api_create_instance():
    ...
    res = manager.create_instance(data['server_id'], int(data['game_port']),
                                  int(data.get('query_port')) if data.get('query_port') else None,
                                  g.user.get('id'), g.user.get('username'))
...
if __name__ == '__main__':
    if USE_HTTPS and os.path.exists('certs/server.crt'):
        app.run(host='0.0.0.0', port=HTTPS_PORT, ssl_context=('certs/server.crt', 'certs/server.key'))
    else:
